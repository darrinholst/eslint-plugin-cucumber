  ...
  function formatFunctionHead(fn) {
    ...
    const paramsRightParen = sourceCode.getTokenBefore(sourceCode.getTokenBefore(fn.body));
        ...
        if (fn.params.length > 0) {
            paramsFullText = `(${ sourceCode.text.slice(fn.params[0].start, fn.params[fn.params.length -1].end) })`;
        }
    ...
    return {
        CallExpression(node) {
            const name = node.callee && node.callee.property ? node.callee.property.name : node.callee.name;
